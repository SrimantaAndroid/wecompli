...
class QrScanViewHolder(itemView:View): RecyclerView.ViewHolder(itemView) {
    fun Bindview(elementdetailsrow: ElementDetailsRow, checkElementDetailsActivity: CheckElementDetailsActivity, position: Int) {
            ...
            btn_qecodescan.typeface=deviceResolution.getbebas(checkElementDetailsActivity)
            tv_checname.setText(elementdetailsrow.checkName)
            tv_checkdescription.setText(elementdetailsrow.checkNote)
        btn_qecodescan.setOnClickListener {
            checkElementDetailsActivity.startscan(elementdetailsrow, position)
    ...
    val btn_qecodescan: Button =itemView.findViewById(R.id.btn_qecodescan)
    val ll_bgqrcode: LinearLayout =itemView.findViewById(R.id.ll_bgqrcode)
